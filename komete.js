...
function findMaxCommets(eu_db, us_db, jp_db, sa_db, year) {
    ...
    var us = formatUs(us_db)
    var eu = formatEu(eu_db)
    var jp = formatJp(jp_db)
    var sa = formatSa(sa_db)
...
function filter(formatedDb, filterYear){
...
function getData(baza, year){
    var filter = filter(baza, year)
    return sortiraj(filter)
}
